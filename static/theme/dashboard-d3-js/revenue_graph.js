...
function dailyRevenueGraph() {
  ...
  if (getMax(revenueGraphDailyData,"revenue") > getMax(closedCaseData,"revenue"))
  {revenueDailyGraphYCoordinate.domain([0, d3.max(revenueGraphDailyData, function (d) { return d.revenue; })]);}
  else
  {revenueDailyGraphYCoordinate.domain([0, d3.max(proposedRevenueGraphDailyData, function (d) { return d.revenue; })]);}
  // revenueDailyGraphYCoordinate.domain([0, d3.max(revenueGraphDailyData, function (d) { return d.revenue; })]);
  ...
  .style("top", (d3.event.pageY - 28 - 2960) + "px")
  ...
  //for proposed revenue line
    proposedRevenueGraphDailyData.forEach(function (proposedRevenueGraphDailyData) {

      proposedRevenueGraphDailyData.date = proposedRevenueGraphDailyData.date;
    
      proposedRevenueGraphDailyData.revenue = +proposedRevenueGraphDailyData.revenue;
    
      });
    
      // Scale the range of the data
    
      // revenueDailyGraphXCoordinate.domain(d3.extent(proposedRevenueGraphDailyData, function (d) { return d.date; }));
      ...
      .attr("d", revenueDailyGraphAxisLine(proposedRevenueGraphDailyData));
    
      // Add the scatterplot
    
      revenueGraphSVG.selectAll("dot")
    
      .data(proposedRevenueGraphDailyData)
...
function monthlyRevenueGraph() {
  ...
  {revenueMonthGraphYCoordinate.domain([0, d3.max(proposedMonthlyRevenue, function (d) { return d.revenue; })]);}
  // revenueMonthGraphYCoordinate.domain([0, d3.max(monthlyRevenue, function (d) { return d.revenue; })]);
  ...
  // proposed monthly data

  //for proposed revenue line
  proposedMonthlyRevenue.forEach(function (proposedMonthlyRevenueData) {

    proposedMonthlyRevenueData.month = new Date(proposedMonthlyRevenueData.month);
  
    proposedMonthlyRevenueData.revenue = +proposedMonthlyRevenueData.revenue;
  
    });
  
    // Scale the range of the data
  
    // revenueMonthGraphXCoordinate.domain(d3.extent(proposedMonthlyRevenue, function (d) { return new Date(d.month); }));
    ...
    revenueGraphSVG.append("path")
      ...
      .attr("class", "line")
      .attr("id","line-2")
      .attr("d", revenueMonthlyGraphAxisLine(proposedMonthlyRevenue));
  
    // Add the scatterplot
  
    revenueGraphSVG.selectAll("dot")
  
      .data(proposedMonthlyRevenue)
