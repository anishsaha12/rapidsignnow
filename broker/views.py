...
def get_notifications(broker):

    unread_statuses = StatusUpdate.objects.filter(case__created_by=broker).order_by('-timestamp')[:15]
    ...
    for status in unread_statuses:
        ...
        if not status.read_by_broker:
            unread_count += 1

        status_notifications.append(notification)

    
    unread_acceptances = CaseAcceptanceUpdate.objects.filter(case__created_by=broker).order_by('-timestamp')[:15]
    ...
    for acceptance in unread_acceptances:
        ...
        notification['read'] = acceptance.read_by_broker
        notification['type'] = 'acceptance'

        if not acceptance.read_by_broker:
            unread_count += 1
...
@permission_required('broker.can_view_broker',raise_exception=True)
def create_new_case(request):
    ...
    context['notifications'] = get_notifications(broker)
    ...
    if request.POST:
        if request.POST.get('context') == 'download-document':
            ...
            new_case = Case(investigator=investigator, law_firm=law_firm, status=status, client_address=new_address,
                            ...
                            client_language=client_language, created_by=broker, adult_clients=adult_clients,
            ...
            new_status_update = StatusUpdate(user=broker.user, status='Case opened', case=new_case,read_by_investigator=False,read_by_master_broker=False, updated_by='BR')
            new_status_update.save()

            new_status_update = StatusUpdate(user=broker.user, status=status, case=new_case,read_by_investigator=False,read_by_master_broker=False, updated_by='BR')
            new_status_update.save()
            ...
            if investigator is not None:
                ...
                try:
                    # if new_case.client_address.gmaps_link :
                    #     message = EmailMessage('Case Assigned', 'The case ' + new_case.name + ' has been assigned to you by ' + broker.user.first_name +
                    #         ' ' + broker.user.last_name + '. Please visit ' + shortened_url + ' and view the details. Visit the client at this address -> '+ new_address.gmaps_link,
            ...
            return HttpResponseRedirect('/broker/all-cases/')

    return render(request, 'broker/create_new_case.html', context)
...
def show_investigator_details(request):
    ...
    return render(request, 'broker/investigator_snippet.html', context)
...
def all_cases(request):
    ...
    cases = []
    law_firms = LawFirm.objects.order_by('-pk')
    ...
    if request.method == 'GET' :
        cases = Case.objects.filter(created_by=broker).order_by('-pk')
        ...
        if 'filtering' in  request.GET and request.GET['filtering'] == 'on':
            ...
            if 'search' in request.GET and request.GET['search'] != '' :
                ...
                # for case in cases:
                #     if case.name in search:
                #         searched_case_names.append(case)
                # for investigator in investigators:
                #     investigator_name = investigator.user.first_name + ' ' + investigator.user.last_name
                #     if investigator_name in search:
                #         searched_investigators.append(investigator.pk) 
                
                # for law_firm in law_firms:
                #     if law_firm.name in search:
                #         searched_law_firms.append(law_firm.pk)
                ...
                context['search'] = search_url.replace('+',' ')
                cases_1=[]
                cases_2=[]
                cases_3=[]
                cases_4=[]
                
                # if len(searched_case_names):
                #     for case in searched_case_names:
                #        cases_0 =  cases_0.filter(pk = case.pk)
                # if len(searched_law_firms):
                #     for law_firm in searched_law_firms:
                #         cases_0 = cases_0.filter(law_firm = law_firm)
                # if len(searched_investigators):
                #     for investigator in searched_investigators:
                #        cases_0 = cases_0.filter(investigator = investigator)
                ...
                if len(q_array):
                    cases_4 = cases.filter(reduce(OR, q_array))  

                if len(cases_3):
                    cases = reduce(OR,(cases_1,cases_2,cases_3,cases_4))
                else:
                    cases = reduce(OR,(cases_1,cases_2,cases_4))
    ...
    if no_of_cases != 'All':        
        paginator = Paginator(cases, no_of_cases) # Show 50 contacts per page
        print (no_of_cases)
    ...
    try:
        ...
        print "No case"
    return render(request, 'broker/all_cases.html', context)
...
def change_status(request):
    ...
    if status.lower() == 'signature obtained':  
        if extra_info:
            ...
            if "adult_client_names" in json_payload:
                # case.adult_clients = str(json_payload["adult_client_names"])
            ...
            if "child_client_names" in json_payload:
                # case.child_clients = str(json_payload["child_client_names"])
    ...
    new_status_update = StatusUpdate(user=broker.user, status=status, case=case, extra_info=extra_info,
                                     read_by_investigator=False,read_by_master_broker=False, updated_by='BR')
    new_status_update.save()
...
def status_mail(case,request):
    ...
    try:
        msg =  EmailMessage('Status Changed', 'Hi '+ investigator.user.first_name +',<br><br> The status of ' + case.name + ' has been changed to ' + case.status +' by ' + broker.user.first_name +
            ' ' + broker.user.last_name + '. Please visit <a href= "http://' + request.META['HTTP_HOST'] + '/investigator/case-details/' +
    ...
    try:
        message = twilio_client.api.account.messages.create(to=phone_number,
                                             from_=settings.TWILIO_OUTGOING_PHONE_NUMBER,
                                             body='The status of ' + case.name + ' has been changed to ' + case.status +' by ' + broker.user.first_name +
              ' ' + broker.user.last_name + '. Please visit ' + shortened_url + ' and view the details. ')
...
def case_details(request, case_id):
    ...
    print "Case id: %d Broker ID: %d"%(case_instance.created_by.id,broker.id) 
    ...
    client_home_phone = case_instance.client_home_phone.replace('-', '').replace(' ', '')
    context['client_home_phone'] = client_home_phone
    return render(request, 'broker/case_details.html', context)
...
def my_profile(request):
    ...
    return render(request, 'broker/my_profile.html', context)
...
def mark_as_read(request):
    ...
    if request_type == 'status':
        ...
        status_update_instance.read_by_broker = True
        ...
        acceptance_instance.read_by_broker = True
...
def change_password(request):
    ...
    context['notifications'] = get_notifications(broker)
    return render(request, 'broker/change_password.html', context)
...
def get_all_notifications(request):
    ...
    unread_statuses = StatusUpdate.objects.filter(case__created_by=broker).order_by('-timestamp')
    status_notifications = []
    unread_count = 0
    ...
    unread_acceptances = CaseAcceptanceUpdate.objects.filter(case__created_by=broker).order_by('-timestamp')
    acceptance_notifications = []
    ...
    return render(request, 'broker/all_notifications.html', context)
...
def get_case_final_status(case):
    ...
    # if len(important_updates) > 0:
    #     important_updates.sort(key=lambda d: d.timestamp, reverse=True)
    #     case_result = important_updates[0].status
...
def mark_case(request):
    ...
    if request.POST['mark'] == "mark":
        if case.status == "Closed":
            ...
            try:
                send_mail('Attention Required !', '' + broker.user.first_name + ' ' + broker.user.last_name + ' has marked the case ' + case.name +' as attention required which was assigned to '+ case.investigator.user.first_name +' ' +case.investigator.user.last_name  + '. Please visit http://' + request.META['HTTP_HOST'] + '/investigator/case-details/' +
            ...
            try:
                message = twilio_client.api.account.messages.create(to=phone_number,
                                                    from_=settings.TWILIO_OUTGOING_PHONE_NUMBER,
                                                    body='' + broker.user.first_name + ' ' + broker.user.last_name + ' has marked the case ' + case.name +' as attention required which was assigned to '+ case.investigator.user.first_name +' ' +case.investigator.user.last_name  + '. Please visit ' + shortened_url + ' and view the details. ')
            except:
                pass
            print "case marked"
...
def dashboard(request):
    ...
    # in_progress = 
    total_cases  = Case.objects.filter(created_by = broker).filter(~Q(status= "Duplicate delete"))
    if not total_cases:
        context["no_data"] = True
        #investigators ranking
        ...
        signature_obtained = Case.objects.filter(created_by = broker).filter(is_signature_obtained = True)
        ...
        signature_not_obtained = Case.objects.filter(created_by = broker).filter(Q(is_signature_obtained = False) & Q(did_investigator_travel = True))
        ...
        for day in range(0, 31):
            ...
            cases_added_count = cases_added_count + cases_on_current_date.count()
            cases_closed_count = cases_closed_count + cases_on_current_date_closed.count()
        
        percentage_of_cases_closed = 0
        if cases_added_count != 0 and cases_closed_count != 0:
            percentage_of_cases_closed = round(float(cases_closed_count * 100 )/ cases_added_count,2)
        context["percentage_of_cases_closed"] = percentage_of_cases_closed
